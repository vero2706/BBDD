...
SELECT DISTINCT CONCAT(a.nombre, 5, CAST(a.id_artista as char)), a.nombre 
FROM artistas a
INNER JOIN artistas_grupo ag ON a.id_artista = ag.id_artista
INNER JOIN grupo g ON g.id_grupo = ag.id_grupo;
...
FROM grupo g
JOIN albums al ON g.id_grupo = al.id_grupo
JOIN canciones c ON al.id_album = c.album;
