-- 1. Contar el número de líneas por pedido (de la tabla orderdetails)
-- 2. Cada pedido puede constar de diferentes lotes o subpedidos (cuyo detalle nos lo da el detalle del pedido)

SELECT OrderId, COUNT(*) as LineasPorPedido FROM orderdetails GROUP BY OrderId;

-- 3. Calcular el número de dinero gastado en cada pedido

select orderID, SUM(Quantity*UnitPrice) as DineroGastado FROM orderDetails GROUP BY OrderID;

-- 4. Hacer una lista del número de clientes de cada país

SELECT COUNT(CustomerID) as NúmeroClientes, Country FROM Customers GROUP BY Country;

SELECT COUNT(*), Country FROM Customers GROUP BY Country;

-- 5. Ordenar la lista anterior de forma descendente
...
-- 6. Hacer un listado del número de pedidos por cliente 

select customerId, count(*) as NúmeroPedidos from orders group by customerid;

-- 7. Contar el número de pedidos por cliente de los clientes de londres
...
-- 8. Ordernar la lista anterior por orden de pedidos (de más a menos)  

select o.customerId, count(*) as PedidosPorCliente from orders o, customers c
where c.customerId = o.customerId AND c.city = 'london'
group by o.customerid ORDER BY PedidosPorCliente desc;

-- 9. En la tabla empleados Employees tenemos el puesto que ocupa cada empleado. Necesitamos un listado del número de personas que ocupan cada puesto
